...
namespace ManoMotion.TryOn
{
    ...
    public class WristInfoGizmo : MonoBehaviour
    {
        ...
        public void ShowWristInformation(HandInfo handInfo, int handIndex)
        {
            ...
            Vector3 leftPos = ManoUtils.Instance.CalculateNewPositionWithDepth(wristInfo.leftPoint, distance);
            Vector3 rightPos = ManoUtils.Instance.CalculateNewPositionWithDepth(wristInfo.rightPoint, distance);
            if (useSmoothing)
            {
                leftPos = leftFilter.Filter(leftPos);
                rightPos = rightFilter.Filter(rightPos);
            }
            leftWrist3D.transform.position = leftPos;
            rightWrist3D.transform.position = rightPos;
        }
