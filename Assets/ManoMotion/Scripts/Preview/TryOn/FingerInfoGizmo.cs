...
namespace ManoMotion.TryOn
{
    ...
    public class FingerInfoGizmo : MonoBehaviour
    {
        ...
        public void ShowFingerInformation(HandInfo handInfo, int handIndex)
        {
            ...
            Vector3 leftPos = ManoUtils.Instance.CalculateNewPositionWithDepth(fingerInfo.leftPoint, distance);
            Vector3 rightPos = ManoUtils.Instance.CalculateNewPositionWithDepth(fingerInfo.rightPoint, distance);
            if (useSmoothing)
            {
                leftPos = leftFilter.Filter(leftPos);
                rightPos = rightFilter.Filter(rightPos);
            }
            leftFingerPoint3D.transform.position = leftPos;
            rightFingerPoint3D.transform.position = rightPos;
        }
