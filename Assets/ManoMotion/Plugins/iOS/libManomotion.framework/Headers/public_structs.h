...
struct CameraInfo2 {
	...
	float radial_coeffs[RADIAL_COEFFS];

	float tangential_coeffs[TANGENTIAL_COEFFS];

	//Camera extrinsics
	float extrinsic_rotation[EXTRINSIC_ROTATION];

	float extrinsic_translation[EXTRINSIC_TRANSLATION];
...
struct ManoMotionStruct {
	...
	cv::Mat lv_lh_mask; // used for returning the hand masks
	cv::Mat lv_rh_mask;
	cv::Mat rv_lh_mask; // used for returning the hand masks
	cv::Mat rv_rh_mask;
